...
>
<channel>
    <title><?php bloginfo_rss('name'); _e(' Downloads RSS Feed', 'wp-cybertour'); ?></title>
    <atom:link href="<?php self_link(); ?>" rel="self" type="application/rss+xml" />
    <link><?php echo get_option('download_page_url'); ?></link>
    <description><?php bloginfo_rss('name'); _e(' Downloads RSS Feed', 'wp-cybertour'); ?></description>
    ...
    <?php if($files): ?>
        <?php foreach($files as $file): ?>
            <item>
                ...
                <description><![CDATA[<?php echo stripslashes($file->file_des); ?><br /><br /><?php printf(__('File Size: %s', 'wp-cybertour'), format_filesize($file->file_size)); ?><br /><?php printf(__('File Hits: %s', 'wp-cybertour'), number_format_i18n($file->file_hits)); ?><br /><?php printf(__('File Last Updated: %s', 'wp-cybertour'), mysql2date(get_option('time_format').' '.get_option('date_format'), gmdate('Y-m-d H:i:s', $file->file_updated_date))); ?>]]></description>
                </item>
        <?php endforeach; ?>
    <?php endif; ?>
</channel>
