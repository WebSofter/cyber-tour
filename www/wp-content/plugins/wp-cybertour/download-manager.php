...
switch($file_sortby) {
	case 'id':
		$file_sortby = 'file_id';
		$file_sortby_text = __('File ID', 'wp-cybertour');
		break;
	case 'file':
		$file_sortby = 'file';
		$file_sortby_text = __('File', 'wp-cybertour');
		break;
	case 'size':
		$file_sortby = '(file_size+0.00)';
		$file_sortby_text = __('File Size', 'wp-cybertour');
		break;
	case 'category':
		$file_sortby = 'file_category';
		$file_sortby_text = __('File Category', 'wp-cybertour');
		break;
	case 'hits':
		$file_sortby = 'file_hits';
		$file_sortby_text = __('File Hits', 'wp-cybertour');
		break;
	case 'permission':
		$file_sortby = 'file_permission';
		$file_sortby_text = __('File Permission', 'wp-cybertour');
		break;
	case 'date':
		$file_sortby = 'FROM_UNIXTIME(file_date)';
		$file_sortby_text = __('File Date', 'wp-cybertour');
		break;
	case 'updated_date':
		$file_sortby = 'FROM_UNIXTIME(file_updated_date)';
		$file_sortby_text = __('File Updated Date', 'wp-cybertour');
		break;
	case 'last_downloaded_date':
		$file_sortby = 'FROM_UNIXTIME(file_last_downloaded_date)';
		$file_sortby_text = __('File Last Downloaded Date', 'wp-cybertour');
		break;
	case 'name':
	default:
		$file_sortby = 'file_name';
		$file_sortby_text = __('File Name', 'wp-cybertour');
}
...
switch($file_sortorder) {
	case 'desc':
		$file_sortorder = 'DESC';
		$file_sortorder_text = __('Descending', 'wp-cybertour');
		break;
	case 'asc':
	default:
		$file_sortorder = 'ASC';
		$file_sortorder_text = __('Ascending', 'wp-cybertour');
}
...
if(!empty($_POST['do'])) {
	...
	switch($_POST['do']) {
		// Edit File
		case __('Edit File', 'wp-cybertour'):
			check_admin_referer('wp-cybertour_edit-file');
			$file_size_sql = '';
			$file_sql = '';
			...
			switch($file_type) {
				case -1:
					...
					if(is_remote_file($file)) {
						...
						$file_size = filesize($plugin_dirname.$file);
					}
					break;
				case 0:
				...
				case 1:
					if( $_FILES['file_upload']['size'] > get_max_upload_size() ) {
						...
						if(is_uploaded_file($_FILES['file_upload']['tmp_name'])) {
							...
							if(move_uploaded_file($_FILES['file_upload']['tmp_name'], $plugin_dirname.$file_upload_to.basename($_FILES['file_upload']['name']))) {
				...
				case 2:
					...
					if ( is_file_remote_valid( $file ) ) {
						...
						$text = '<p style="color: red;">' . __( 'There Is An Error Parsing Remote File URL', 'wp-cybertour' ) . '</p>';
					}
					break;
			}
			if ( empty( $text ) ) {
				...
				if(!$editfile) {
					$text = '<p style="color: red;">'.sprintf(__('Error In Editing File \'%s (%s)\'', 'wp-cybertour'), $file_name, $file).'</p>';
				} else {
					$text = '<p style="color: green;">'.sprintf(__('File \'%s (%s)\' Edited Successfully', 'wp-cybertour'), $file_name, $file).'</p>';
				}
			}
			break;
		// Delete File
		case __('Delete File', 'wp-cybertour');
			check_admin_referer('wp-cybertour_delete-file');
			$file_id  = ! empty( $_POST['file_id'] ) ? intval( $_POST['file_id'] ) : 0;
			...
			//if($unlinkfile == 1) {
				//
				if(is_dir($plugin_dirname.$file) && !del_folder($plugin_dirname.$file)) {
					$text = '<p style="color: red;">'.sprintf(__('Error In Deleting Folder \'%s (%s)\' From Server', 'wp-cybertour'), $file_name, $file).'</p>';
				} else {
					$text = '<p style="color: green;">'.sprintf(__('Folder \'%s (%s)\' Deleted From Server Successfully', 'wp-cybertour'), $file_name, $file).'</p>';
				}
				if(is_file($plugin_dirname.$file) && !unlink($plugin_dirname.$file)) {
					$text = '<p style="color: red;">'.sprintf(__('Error In Deleting File \'%s (%s)\' From Server', 'wp-cybertour'), $file_name, $file).'</p>';
				} else {
					$text = '<p style="color: green;">'.sprintf(__('File \'%s (%s)\' Deleted From Server Successfully', 'wp-cybertour'), $file_name, $file).'</p>';
				}
			//}
			$deletefile = $wpdb->query("DELETE FROM $wpdb->downloads WHERE file_id = $file_id");
			if(!$deletefile) {
				$text .= '<p style="color: red;">'.sprintf(__('Error In Deleting File \'%s (%s)\'', 'wp-cybertour'), $file_name, $file).'</p>';
			} else {
				$text .= '<p style="color: green;">'.sprintf(__('File \'%s (%s)\' Deleted Successfully', 'wp-cybertour'), $file_name, $file).'</p>';
			}
			break;
	}
}
...
switch($mode) {
	...
	case 'edit':
		...
		<form method="post" action="<?php echo admin_url('admin.php?page='.plugin_basename(__FILE__).'&amp;mode=edit&amp;id='.intval($file->file_id)); ?>" enctype="multipart/form-data">
			...
			<?php wp_nonce_field('wp-cybertour_edit-file'); ?>
			<div class="wrap">
				<h2><?php _e('Edit A File', 'wp-cybertour'); ?></h2>
				<table class="form-table">
					<tr>
						<td valign="top"><strong><?php _e('File:', 'wp-cybertour') ?></strong></td>
						<td>
							//File Name
							<input type="radio" id="file_type_-1" name="file_type" value="-1" checked="checked" />&nbsp;&nbsp;<label for="file_type_-1"><?php _e('Current File:', 'wp-cybertour'); ?>&nbsp;<strong dir="ltr"><?php echo stripslashes($file->file); ?></strong></label>&nbsp;
							<br /><br />
							//Browse File
							<input type="radio" id="file_type_0" name="file_type" value="0" />&nbsp;&nbsp;<label for="file_type_0"><?php _e('Browse File:', 'wp-cybertour'); ?></label>&nbsp;
							...
							<br /><small><?php printf(__('Please upload the file to \'%s\' directory first.', 'wp-cybertour'), $file_path); ?></small>
							<br /><br />
							//Upload File
							<input type="radio" id="file_type_1" name="file_type" value="1" />&nbsp;&nbsp;<label for="file_type_1"><?php _e('Upload File:', 'wp-cybertour'); ?></label>&nbsp;
							<input type="file" name="file_upload" size="25" onclick="document.getElementById('file_type_1').checked = true;" dir="ltr" />&nbsp;&nbsp;<?php _e('to', 'wp-cybertour'); ?>&nbsp;&nbsp;
							...
							//Remote File
							<br /><br />
							<input type="radio" id="file_type_2" name="file_type" value="2" />&nbsp;&nbsp;<label for="file_type_2"><?php _e('Remote File:', 'wp-cybertour'); ?></label>&nbsp;
							...
							<br /><small><?php _e('Please include http:// or ftp:// in front.', 'wp-cybertour'); ?></small>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td><strong><?php _e('File Size:', 'wp-cybertour') ?></strong></td>
						<td><?php echo format_filesize($file->file_size); ?><br /><input type="text" size="10" name="file_size" value="<?php echo $file->file_size; ?>" />&nbsp;<?php _e('bytes', 'wp-cybertour'); ?><br /><input type="checkbox" id="auto_filesize" name="auto_filesize" value="1" checked="checked" />&nbsp;<label for="auto_filesize"><?php _e('Auto Detection Of File Size', 'wp-cybertour') ?></label></td>
					</tr>
					<tr>
						<td valign="top"><strong><?php _e('File Hits:', 'wp-cybertour') ?></strong></td>
						<td><?php printf(_n('%s hit', '%s hits', number_format_i18n($file->file_hits),'wp-cybertour'), number_format_i18n($file->file_hits)) ?><br /><input type="text" size="6" maxlength="10" name="file_hits" value="<?php echo $file->file_hits; ?>" /><br /><input type="checkbox" id="reset_filehits" name="reset_filehits" value="1" />&nbsp;<label for="reset_filehits"><?php _e('Reset File Hits', 'wp-cybertour') ?></label></td>
					</tr>
					<tr>
						<td valign="top"><strong><?php _e('File Date:', 'wp-cybertour') ?></strong></td>
						<td><?php _e('Existing Timestamp:', 'wp-cybertour') ?> <?php echo mysql2date(sprintf('%s @ %s', get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $file->file_date)); ?><br /><?php file_timestamp($file->file_date); ?><br /><input type="checkbox" id="edit_filetimestamp" name="edit_filetimestamp" value="1" />&nbsp;<label for="edit_filetimestamp"><?php _e('Edit Timestamp', 'wp-cybertour') ?></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="edit_usetodaydate" value="1" onclick="file_usetodaydate();" />&nbsp;<label for="edit_usetodaydate"><?php _e('Use Today\'s Date', 'wp-cybertour') ?></label></td>
					</tr>
					<tr>
						<td valign="top"><strong><?php _e('File Updated Date:', 'wp-cybertour') ?></strong></td>
					...
					<tr>
						<td><strong><?php _e('File Last Downloaded Date:', 'wp-cybertour') ?></strong></td>
					...
					<tr>
						...
						<td>
							<select name="file_permission" size="1">
								<option value="-2" <?php selected('-2', $file->file_permission); ?>><?php _e('Hidden', 'wp-cybertour'); ?></option>
								<option value="-1" <?php selected('-1', $file->file_permission); ?>><?php _e('Everyone', 'wp-cybertour'); ?></option>
								<option value="0" <?php selected('0', $file->file_permission); ?>><?php _e('Registered Users Only', 'wp-cybertour'); ?></option>
								<option value="1" <?php selected('1', $file->file_permission); ?>><?php _e('At Least Contributor Role', 'wp-cybertour'); ?></option>
								<option value="2" <?php selected('2', $file->file_permission); ?>><?php _e('At Least Author Role', 'wp-cybertour'); ?></option>
								<option value="7" <?php selected('7', $file->file_permission); ?>><?php _e('At Least Editor Role', 'wp-cybertour'); ?></option>
								<option value="10" <?php selected('10', $file->file_permission); ?>><?php _e('At Least Administrator Role', 'wp-cybertour'); ?></option>
							</select>
						</td>
					</tr>
					<tr>
						<td colspan="2" align="center"><input type="submit" name="do" value="<?php _e('Edit File', 'wp-cybertour'); ?>"  class="button" />&nbsp;&nbsp;<input type="button" name="cancel" value="<?php _e('Cancel', 'wp-cybertour'); ?>" class="button" onclick="javascript:history.go(-1)" /></td>
					</tr>
				</table>
			</div>
		</form>
	...
	case 'delete':
		...
		<form method="post" action="<?php echo admin_url('admin.php?page='.plugin_basename(__FILE__)); ?>">
			...
			<?php wp_nonce_field('wp-cybertour_delete-file'); ?>
			<div class="wrap">
				<h2><?php _e('Delete A File', 'wp-cybertour'); ?></h2>
				<br style="clear" />
				<table class="widefat">
					<tr>
						<td valign="top"><strong><?php _e('File:', 'wp-cybertour') ?></strong></td>
					...
					<tr class="alternate">
						<td><strong><?php _e('File Hits', 'wp-cybertour'); ?></strong></td>
						<td><?php echo number_format_i18n($file->file_hits); ?> <?php _e('hits', 'wp-cybertour'); ?></td>
					</tr>
					<tr>
						<td><strong><?php _e('File Date', 'wp-cybertour'); ?></strong></td>
						<td><?php echo mysql2date(sprintf('%s @ %s', get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $file->file_date)); ?></td>
					</tr>
					<tr class="alternate">
						<td><strong><?php _e('File Updated Date:', 'wp-cybertour') ?></strong></td>
						<td><?php echo mysql2date(sprintf('%s @ %s', get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $file->file_updated_date)); ?></td>
					</tr>
					<tr>
						<td><strong><?php _e('File Last Downloaded Date:', 'wp-cybertour') ?></strong></td>
					...
					<!--
						<tr>
							<td colspan="2" align="center"><input type="checkbox" id="unlinkfile" name="unlinkfile" value="1" />&nbsp;<label for="unlinkfile"><?php _e('Delete File From Server?', 'wp-cybertour'); ?></label></td>
						</tr>
					-->
					<?php endif; ?>
					<tr class="alternate">
						<td colspan="2" align="center"><input type="submit" name="do" value="<?php _e('Delete File', 'wp-cybertour'); ?>" class="button"  onclick="return confirm('You Are About To The Delete This File \'<?php echo esc_attr( removeslashes( $file->file_name ) ); ?> (<?php echo esc_attr( removeslashes( $file->file ) ); ?>)\'.\nThis Action Is Not Reversible.\n\n Choose \'Cancel\' to stop, \'OK\' to delete.')"/>&nbsp;&nbsp;<input type="button" name="cancel" value="<?php _e('Cancel', 'wp-cybertour'); ?>" class="button" onclick="javascript:history.go(-1)" /></td>
					</tr>
				</table>
			</div>
		</form>
	...
	default:
		...
		$total_bandwidth = $wpdb->get_var("SELECT SUM(file_hits*file_size) AS total_bandwidth FROM $wpdb->downloads WHERE file_size != '".__('unknown', 'wp-cybertour')."'");
		$total_filesize = $wpdb->get_var("SELECT SUM(file_size) AS total_filesize FROM $wpdb->downloads WHERE file_size != '".__('unknown', 'wp-cybertour')."'");
		...
		<!-- Manage Downloads -->
		<div class="wrap">
			<h2><?php _e('Manage Downloads', 'wp-cybertour'); ?></h2>
			<h3><?php _e('Downloads', 'wp-cybertour'); ?></h3>
			<p><?php printf(__('Displaying <strong>%s</strong> To <strong>%s</strong> Of <strong>%s</strong> Files', 'wp-cybertour'), number_format_i18n($display_on_page), number_format_i18n($max_on_page), number_format_i18n($get_total_files)); ?></p>
			<p><?php printf(__('Sorted By <strong>%s</strong> In <strong>%s</strong> Order', 'wp-cybertour'), $file_sortby_text, $file_sortorder_text); ?></p>
			<table class="widefat">
				<thead>
				<tr>
					<th><?php _e('ID', 'wp-cybertour'); ?></th>
					<th><?php _e('File', 'wp-cybertour'); ?></th>
					<th><?php _e('Size', 'wp-cybertour'); ?></th>
					<th><?php _e('Hits', 'wp-cybertour'); ?></th>
					<th><?php _e('Permission', 'wp-cybertour'); ?></th>
					<th><?php _e('Category', 'wp-cybertour'); ?></th>
					<th><?php _e('Date/Time Added', 'wp-cybertour'); ?></th>
					<th colspan="2"><?php _e('Action', 'wp-cybertour'); ?></th>
				</tr>
				</thead>
				<?php
				if($files) {;
					...
					foreach($files as $file) {
						...
						foreach (glob("{$file_name}/*.html") as $file) {
							$link ='/wp-content'.explode('wp-content', $file)[1];
						}

						echo "<tr$style>\n";
						echo '<td valign="top">'.number_format_i18n($file_id).'</td>'."\n";
						echo "<td>
								<a href='{$link}' target='_blank'>$file_nicename</a><br />
								<input style='width:100%' type='text' value='".site_url()."{$link}'/><br/>
								".sprintf(__('Last Updated: %s, %s', 'wp-cybertour'), $file_updated_time, $file_updated_date)."</i><br /><i>".sprintf(__('Last Downloaded: %s, %s', 'wp-cybertour'), $file_last_downloaded_time, $file_last_downloaded_date)."</i></td>\n";
						...
						//echo "<td style=\"text-align: center;\"><a href=\"$base_page&amp;mode=edit&amp;id=$file_id\" class=\"edit\">".__('Edit', 'wp-cybertour')."</a></td>\n";
						echo "<td style=\"text-align: center;\"><a href=\"$base_page&amp;mode=delete&amp;id=$file_id\" class=\"delete\">".__('Delete', 'wp-cybertour')."</a></td>\n";
						echo '</tr>';
						$i++;
					}
				} else {
					echo '<tr><td colspan="9" align="center"><strong>'.__('No Files Found', 'wp-cybertour').'</strong></td></tr>';
				}
				?>
			</table>
			<!-- <Paging> -->
				...
				<table class="widefat">
					<tr>
						<td align="<?php echo ('rtl' == $text_direction) ? 'right' : 'left'; ?>" width="50%">
							...
							if($file_page > 1 && ((($file_page*$file_perpage)-($file_perpage-1)) <= $get_total_files)) {
								echo '<strong>&laquo;</strong> <a href="'.$base_page.'&amp;filepage='.($file_page-1).$file_sort_url.'" title="&laquo; '.__('Previous Page', 'wp-cybertour').'">'.__('Previous Page', 'wp-cybertour').'</a>';
							} else {
								echo '&nbsp;';
							}
							?>
						...
						<td align="<?php echo ('rtl' == $text_direction) ? 'left' : 'right'; ?>" width="50%">
							...
							if($file_page >= 1 && ((($file_page*$file_perpage)+1) <= $get_total_files)) {
								echo '<a href="'.$base_page.'&amp;filepage='.($file_page+1).$file_sort_url.'" title="'.__('Next Page', 'wp-cybertour').' &raquo;">'.__('Next Page', 'wp-cybertour').'</a> <strong>&raquo;</strong>';
							} else {
								echo '&nbsp;';
							}
							?>
					...
					<tr class="alternate">
						<td colspan="2" align="center">
							<?php _e('Pages', 'wp-cybertour'); ?> (<?php echo number_format_i18n($total_pages); ?>):
							<?php
							if ($file_page >= 4) {
								echo '<strong><a href="'.$base_page.'&amp;filepage=1'.$file_sort_url.'" title="'.__('Go to First Page', 'wp-cybertour').'">&laquo; '.__('First', 'wp-cybertour').'</a></strong> ... ';
							}
							if($file_page > 1) {
								echo ' <strong><a href="'.$base_page.'&amp;filepage='.($file_page-1).$file_sort_url.'" title="&laquo; '.__('Go to Page', 'wp-cybertour').' '.number_format_i18n($file_page-1).'">&laquo;</a></strong> ';
							}
							for($i = $file_page - 2 ; $i  <= $file_page +2; $i++) {
								if ($i >= 1 && $i <= $total_pages) {
									if($i == $file_page) {
										...
										echo '<a href="'.$base_page.'&amp;filepage='.($i).$file_sort_url.'" title="'.__('Page', 'wp-cybertour').' '.number_format_i18n($i).'">'.number_format_i18n($i).'</a> ';
									}
								}
							}
							if($file_page < $total_pages) {
								echo ' <strong><a href="'.$base_page.'&amp;filepage='.($file_page+1).$file_sort_url.'" title="'.__('Go to Page', 'wp-cybertour').' '.number_format_i18n($file_page+1).' &raquo;">&raquo;</a></strong> ';
							}
							if (($file_page+2) < $total_pages) {
								echo ' ... <strong><a href="'.$base_page.'&amp;filepage='.($total_pages).$file_sort_url.'" title="'.__('Go to Last Page', 'wp-cybertour'), 'wp-cybertour'.'">'.__('Last', 'wp-cybertour').' &raquo;</a></strong>';
							}
							?>
						</td>
					</tr>
			...
			<form action="<?php echo admin_url('admin.php?page='.plugin_basename(__FILE__)); ?>" method="get">
				<table class="widefat">
					<tr>
						<th><?php _e('Filter Options: ', 'wp-cybertour'); ?></th>
						<td><?php _e('Keywords:', 'wp-cybertour'); ?><input type="text" name="search" size="30" maxlength="200" value="<?php echo esc_attr( $file_search ); ?>" /></td>
					</tr>
					<tr>
						<th><?php _e('Sort Options:', 'wp-cybertour'); ?></th>
						<td>
							<input type="hidden" name="page" value="<?php echo $base_name; ?>" />
							<select name="by" size="1">
								<option value="id"<?php if($file_sortby == 'file_id') { echo ' selected="selected"'; }?>><?php _e('File ID', 'wp-cybertour'); ?></option>
								<option value="file"<?php if($file_sortby == 'file') { echo ' selected="selected"'; }?>><?php _e('File', 'wp-cybertour'); ?></option>
								<option value="name"<?php if($file_sortby == 'file_name') { echo ' selected="selected"'; }?>><?php _e('File Name', 'wp-cybertour'); ?></option>
								<option value="date"<?php if($file_sortby == 'FROM_UNIXTIME(file_date)') { echo ' selected="selected"'; }?>><?php _e('File Date', 'wp-cybertour'); ?></option>
								<option value="updated_date"<?php if($file_sortby == 'FROM_UNIXTIME(updated_date)') { echo ' selected="selected"'; }?>><?php _e('File Updated Date', 'wp-cybertour'); ?></option>
								<option value="last_downloaded_date"<?php if($file_sortby == 'FROM_UNIXTIME(last_downloaded_date)') { echo ' selected="selected"'; }?>><?php _e('File Last Downloaded Date', 'wp-cybertour'); ?></option>
								<option value="size"<?php if($file_sortby == '(file_size+0.00)') { echo ' selected="selected"'; }?>><?php _e('File Size', 'wp-cybertour'); ?></option>
								<option value="category"<?php if($file_sortby == 'file_category') { echo ' selected="selected"'; }?>><?php _e('File Category', 'wp-cybertour'); ?></option>
								<option value="hits"<?php if($file_sortby == 'file_hits') { echo ' selected="selected"'; }?>><?php _e('File Hits', 'wp-cybertour'); ?></option>
								<option value="permission"<?php if($file_sortby == 'file_permission') { echo ' selected="selected"'; }?>><?php _e('File Permission', 'wp-cybertour'); ?></option>
							</select>
							&nbsp;&nbsp;&nbsp;
							<select name="order" size="1">
								<option value="asc"<?php if($file_sortorder == 'ASC') { echo ' selected="selected"'; }?>><?php _e('Ascending', 'wp-cybertour'); ?></option>
								<option value="desc"<?php if($file_sortorder == 'DESC') { echo ' selected="selected"'; } ?>><?php _e('Descending', 'wp-cybertour'); ?></option>
							</select>
							&nbsp;&nbsp;&nbsp;
							<select name="perpage" size="1">
								...
								for($k=10; $k <= 100; $k+=10) {
									if($file_perpage == $k) {
										echo "<option value=\"$k\" selected=\"selected\">".__('Per Page', 'wp-cybertour').": ".number_format_i18n($k)."</option>\n";
									} else {
										echo "<option value=\"$k\">".__('Per Page', 'wp-cybertour').": ".number_format_i18n($k)."</option>\n";
									}
								}
								?>
							</select>
					...
					<tr>
						<td colspan="2" align="center"><input type="submit" value="<?php _e('Go', 'wp-cybertour'); ?>" class="button" /></td>
					</tr>
				</table>
			</form>
		</div>
		...
		<!-- Download Stats -->
		<div class="wrap">
			<h3><?php _e('Download Stats', 'wp-cybertour'); ?></h3>
			<br style="clear" />
			<table class="widefat">
				<tr>
					<th><?php _e('Total Files:', 'wp-cybertour'); ?></th>
					<td><?php echo number_format_i18n($total_file); ?></td>
				</tr>
				<tr class="alternate">
					<th><?php _e('Total Size:', 'wp-cybertour'); ?></th>
					<td><?php echo format_filesize($total_filesize); ?></td>
				</tr>
				<tr>
					<th><?php _e('Total Hits:', 'wp-cybertour'); ?></th>
					<td><?php echo number_format_i18n($total_filehits); ?></td>
				</tr>
				<tr class="alternate">
					<th><?php _e('Total Bandwidth:', 'wp-cybertour'); ?></th>
