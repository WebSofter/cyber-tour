<?php
/*
Plugin Name: 3D Cyber Tour
Plugin URI: https://wsofter.com/projects/cyber-tour
Description: Plugin for creating Cyber 3D tours and share it.
Version: 1.0.0
Author: David Amirkhanov (websofter)
Author URI: https://wsofter.com
Text Domain: cyber-tour
*/
...
### Function: Tours Administration Menu
add_action('admin_menu', 'downloads_menu');
function downloads_menu() {
	add_menu_page(__('Tours', 'wp-cybertour'), __('Tours', 'wp-cybertour'), 'manage_downloads', 'wp-cybertour/download-manager.php', '', 'dashicons-welcome-view-site');

	add_submenu_page('wp-cybertour/download-manager.php', __('Manage Tours', 'wp-cybertour'), __('Manage Tours', 'wp-cybertour'), 'manage_downloads', 'wp-cybertour/download-manager.php');
	add_submenu_page('wp-cybertour/download-manager.php', __('Add Tour', 'wp-cybertour'), __('Add Tour', 'wp-cybertour'), 'manage_downloads', 'wp-cybertour/download-add.php');
	add_submenu_page('wp-cybertour/download-manager.php', __('Download Options', 'wp-cybertour'), __('Download Options', 'wp-cybertour'), 'manage_downloads', 'wp-cybertour/download-options.php');
	//add_submenu_page('wp-cybertour/download-manager.php', __('Download Templates', 'wp-cybertour'), __('Download Templates', 'wp-cybertour'), 'manage_downloads', 'wp-cybertour/download-templates.php');
}


### Function: Enqueue Tours Stylesheets
...
function downloads_stylesheets() {
	if(@file_exists(TEMPLATEPATH.'/download-css.css')) {
		wp_enqueue_style('wp-cybertour', get_stylesheet_directory_uri().'/download-css.css', false, WP_DOWNLOADMANAGER_VERSION, 'all');
	} else {
		wp_enqueue_style('wp-cybertour', plugins_url('wp-cybertour/download-css.css'), false, WP_DOWNLOADMANAGER_VERSION, 'all');
	}
}


### Function: Enqueue Tours Stylesheets In WP-Admin
add_action('admin_enqueue_scripts', 'downloads_stylesheets_admin');
function downloads_stylesheets_admin($hook_suffix) {
	$downloads_admin_pages = array('wp-cybertour/download-manager.php', 'wp-cybertour/download-add.php', 'wp-cybertour/download-options.php', 'wp-cybertour/download-templates.php', 'wp-cybertour/download-uninstall.php');
	if(in_array($hook_suffix, $downloads_admin_pages)) {
		wp_enqueue_style('wp-cybertour-admin', plugins_url('wp-cybertour/download-admin-css.css'), false, WP_DOWNLOADMANAGER_VERSION, 'all');
	}
}
...
?>
	<script type="text/javascript">
		QTags.addButton('ed_wp_downloadmanager', '<?php echo esc_js(__('Download', 'wp-cybertour')); ?>', function() {
			var download_id = jQuery.trim(prompt('<?php echo esc_js(__('Enter File ID (Separate Multiple IDs By A Comma)', 'wp-cybertour')); ?>'));
...
function download_tinymce_addplugin( $plugin_array ) {
	if( WP_DEBUG ) {
		$plugin_array['downloadmanager'] = plugins_url( 'wp-cybertour/tinymce/plugins/downloadmanager/plugin.js?v=' . WP_DOWNLOADMANAGER_VERSION);
	} else {
		$plugin_array['downloadmanager'] = plugins_url( 'wp-cybertour/tinymce/plugins/downloadmanager/plugin.min.js?v= ' . WP_DOWNLOADMANAGER_VERSION);
	}
	return $plugin_array;
}
function download_tinymce_translation($mce_translation) {
	$mce_translation['Enter File ID (Separate Multiple IDs By A Comma)'] = esc_js(__('Enter File ID (Separate Multiple IDs By A Comma)', 'wp-cybertour'));
	$mce_translation['Insert File Download'] = esc_js(__('Insert File Download', 'wp-cybertour'));
...
function download_rss_link() {
	if(is_page() && strpos(get_option('download_page_url'), $_SERVER['REQUEST_URI'])) {
		...
		echo '<link rel="alternate" type="application/rss+xml" title="'.get_bloginfo_rss('name').__(' Tours RSS Feed', 'wp-cybertour').'" href="'.$download_rss_link.'" />'."\n";
	}
}
...
function download_file() {
	...
	if($dl_name === 'rss') {
		load_template(WP_PLUGIN_DIR.'/wp-cybertour/download-rss.php');
		exit;
	}
	if($dl_id > 0 || !empty($dl_name)) {
		...
		if( empty( $file ) ) {
			...
			die(__('Invalid File ID or File Name.', 'wp-cybertour'));
		}
		...
		if( $file_permission === -1 || ( $file_permission === 0 && (int) $user_ID > 0 ) || ($file_permission > 0 && get_wp_user_level() >= $file_permission && (int) $user_ID > 0 ) ) {
			...
			if(!is_remote_file($file_name)) {
				if(!is_file($file_path.$file_name)) {
					header('HTTP/1.0 404 Not Found');
					die(__('File does not exist.', 'wp-cybertour'));
				}
				...
				if(ini_get('allow_url_fopen') && $download_method == 0) {
					...
					if($file_size !== __('unknown', 'wp-cybertour')) {
			...
			_e('You do not have permission to download this file.', 'wp-cybertour');
			exit();
		}
	}
}
...
function file_extension_images() {
	...
	$dir = apply_filters( 'wp_downloadmanager_file_extension_images_path', WP_PLUGIN_DIR . '/wp-cybertour/images/ext' );
	if ( is_dir( $dir ) ) {
		if ( $dh = opendir( $dir ) ) {
...
if(!function_exists('remote_filesize')) {
	function remote_filesize($uri) {
		...
		if(!empty($file_size)) {
			...
			return __('unknown', 'wp-cybertour');
		}
	}
}
...
if ( ! function_exists( 'format_filesize' ) ) {
	function format_filesize($rawSize) {
		...
		if ( $rawSize / 1099511627776 > 1 ) {
			return number_format_i18n( $rawSize/1099511627776, 1 ) . ' ' . __( 'TiB', 'wp-cybertour' );
		} elseif ( $rawSize / 1073741824 > 1 ) {
			return number_format_i18n( $rawSize/1073741824, 1 ) . ' ' . __( 'GiB', 'wp-cybertour' );
		} elseif ( $rawSize / 1048576 > 1 ) {
			return number_format_i18n( $rawSize/1048576, 1 ) . ' ' . __( 'MiB', 'wp-cybertour' );
		} elseif ( $rawSize / 1024 > 1 ) {
			return number_format_i18n( $rawSize/1024, 1 ) . ' ' . __( 'KiB', 'wp-cybertour' );
		} elseif ( $rawSize > 1 ) {
			return number_format_i18n( $rawSize ) . ' ' . __( 'bytes', 'wp-cybertour' );
		} else {
			return __( 'unknown', 'wp-cybertour' );
		}
	}
...
if ( ! function_exists( 'format_filesize_dec' ) ) {
	function format_filesize_dec( $rawSize ) {
		...
		if( $rawSize / 1000000000000 > 1 ) {
			return number_format_i18n( $rawSize/1000000000000, 1 ) . ' ' . __( 'TB', 'wp-cybertour' );
		} elseif ( $rawSize / 1000000000 > 1 ) {
			return number_format_i18n( $rawSize/1000000000, 1 ) .' ' . __( 'GB', 'wp-cybertour' );
		} elseif ( $rawSize / 1000000 > 1 ) {
			return number_format_i18n( $rawSize/1000000, 1 ) . ' ' . __( 'MB', 'wp-cybertour' );
		} elseif ( $rawSize / 1000 > 1 ) {
			return number_format_i18n( $rawSize/1000, 1 ) . ' ' . __( 'KB', 'wp-cybertour' );
		} elseif ( $rawSize > 1 ) {
			return number_format_i18n( $rawSize ) . ' ' . __( 'bytes', 'wp-cybertour' );
		} else {
			return __( 'unknown', 'wp-cybertour' );
...
function get_max_upload_size() {
	...
	*/
	...
	$size_str = trim(ini_get('upload_max_filesize'));
...
### Function: Short Code For Inserting Tours Page Into Page
...
function download_shortcode( $atts ) {
	...
	return __( 'Note: There is a file embedded within this post, please visit this post to download the file.', 'wp-cybertour' );
}
...
function downloads_page($category_id = 0) {
	...
	$download_categories[0] = __('total', 'wp-cybertour');
	...
	if($max_page > 1) {
		...
		if(function_exists('wp_pagenavi')) {
			...
			$output .= '<div class="wp-cybertour-paging">'."\n";
		}
		$output .= '<span class="pages">&#8201;'.sprintf(__('Page %s of %s', 'wp-cybertour'), number_format_i18n($page), number_format_i18n($max_page)).'&#8201;</span>';
		if ($start_page >= 2 && $pages_to_show < $max_page) {
			$output .= '<a href="'.download_page_link(1).'" title="'.__('&laquo; First', 'wp-cybertour').'">&#8201;'.__('&laquo; First', 'wp-cybertour').'&#8201;</a>';
			$output .= '<span class="extend">...</span>';
		}
		if($page > 1) {
			$output .= '<a href="'.download_page_link(($page-1)).'" title="'.__('&laquo;', 'wp-cybertour').'">&#8201;'.__('&laquo;', 'wp-cybertour').'&#8201;</a>';
		}
		...
		if(empty($page) || ($page+1) <= $max_page) {
			$output .= '<a href="'.download_page_link(($page+1)).'" title="'.__('&raquo;', 'wp-cybertour').'">&#8201;'.__('&raquo;', 'wp-cybertour').'&#8201;</a>';
		}
		if ($end_page < $max_page) {
			$output .= '<span class="extend">...</span>';
			$output .= '<a href="'.download_page_link($max_page).'" title="'.__('Last &raquo;', 'wp-cybertour').'">&#8201;'.__('Last &raquo;', 'wp-cybertour').'&#8201;</a>';
		}
		$output .= '</div>';
...
### Function: List Out All Files In Tours Directory
...
function file_permission($file_permission) {
	...
	switch( (int) $file_permission ) {
		case -2:
			$file_permission_name = __('Hidden', 'wp-cybertour');
			break;
		case -1:
			$file_permission_name = __('Everyone', 'wp-cybertour');
			break;
		case 0:
			$file_permission_name = __('Registered Users Only', 'wp-cybertour');
			break;
		case 1:
			$file_permission_name = __('At Least Contributor Role', 'wp-cybertour');
			break;
		case 2:
			$file_permission_name = __('At Least Author Role', 'wp-cybertour');
			break;
		case 7:
			$file_permission_name = __('At Least Editor Role', 'wp-cybertour');
			break;
		case 10:
			$file_permission_name = __('At Least Administrator Role', 'wp-cybertour');
			break;
	}
...
function download_embedded($condition = '', $display = 'both', $sort_by = 'file_id', $sort_order = 'asc', $stream_limit = 0) {
	...
	if($files) {
		...
		if (!is_single() && $stream_limit != 0 && $stream_limit < count($files)) {
			$output .= '<p><a href="' . get_permalink() . '">'.__('More …', 'wp-cybertour').'</a></p>';
		}
...
if(!function_exists('get_most_downloaded')) {
	function get_most_downloaded($limit = 10, $chars = 0, $display = true) {
		...
		if($files) {
			...
			$output = '<li>'.__('N/A', 'wp-cybertour').'</li>'."\n";
		}
		if($display) {
			echo $output;
		} else {
...
### Function: Get Tours By Category ID
...
### Function: Add wp-cybertour General Stats To WP-Stats Page Options
function downloadmanager_page_admin_general_stats($content) {
	$stats_display = get_option('stats_display');
	if( (int)  $stats_display['downloads'] === 1 ) {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_downloads" value="downloads" checked="checked" />&nbsp;&nbsp;<label for="wpstats_downloads">'.__('wp-cybertour', 'wp-cybertour').'</label><br />'."\n";
	} else {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_downloads" value="downloads" />&nbsp;&nbsp;<label for="wpstats_downloads">'.__('wp-cybertour', 'wp-cybertour').'</label><br />'."\n";
	}
	return $content;
}


### Function: Add wp-cybertour Top Recent Stats To WP-Stats Page Options
function downloadmanager_page_admin_recent_stats($content) {
	...
	if( (int) $stats_display['recent_downloads'] === 1) {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_recent_downloads" value="recent_downloads" checked="checked" />&nbsp;&nbsp;<label for="wpstats_recent_downloads">'.sprintf(_n('%s Most Recent Download', '%s Most Recent Tours', $stats_mostlimit, 'wp-cybertour'), number_format_i18n($stats_mostlimit)).'</label><br />'."\n";
	} else {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_recent_downloads" value="recent_downloads" />&nbsp;&nbsp;<label for="wpstats_recent_downloads">'.sprintf(_n('%s Most Recent Download', '%s Most Recent Tours', $stats_mostlimit, 'wp-cybertour'), number_format_i18n($stats_mostlimit)).'</label><br />'."\n";
	}
	return $content;
}


### Function: Add wp-cybertour Top Most/Highest Stats To WP-Stats Page Options
function downloadmanager_page_admin_most_stats($content) {
	$stats_display = get_option('stats_display');
	$stats_mostlimit = (int) get_option('stats_mostlimit');
	if( (int) $stats_display['downloaded_most'] === 1) {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_downloaded_most" value="downloaded_most" checked="checked" />&nbsp;&nbsp;<label for="wpstats_downloaded_most">'.sprintf(_n('%s Most Downloaded File', '%s Most Downloaded Files', $stats_mostlimit, 'wp-cybertour'), number_format_i18n($stats_mostlimit)).'</label><br />'."\n";
	} else {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_downloaded_most" value="downloaded_most" />&nbsp;&nbsp;<label for="wpstats_downloaded_most">'.sprintf(_n('%s Most Downloaded File', '%s Most Downloaded Files', $stats_mostlimit, 'wp-cybertour'), number_format_i18n($stats_mostlimit)).'</label><br />'."\n";
	}
	return $content;
}


### Function: Add wp-cybertour General Stats To WP-Stats Page
function downloadmanager_page_general_stats($content) {
	...
	if( (int) $stats_display['downloads'] === 1 ) {
		...
		$content .= '<p><strong>'.__('wp-cybertour', 'wp-cybertour').'</strong></p>'."\n";
		$content .= '<ul>'."\n";
		$content .= '<li>'.sprintf(_n('<strong>%s</strong> file was added.', '<strong>%s</strong> files were added.', $download_stats->total_files, 'wp-cybertour'), number_format_i18n($download_stats->total_files)).'</li>'."\n";
		$content .= '<li>'.sprintf(_n('<strong>%s</strong> worth of files.', '<strong>%s</strong> worth of files.', $download_stats->total_size, 'wp-cybertour'), format_filesize($download_stats->total_size)).'</li>'."\n";
		$content .= '<li>'.sprintf(_n('<strong>%s</strong> hit was generated.', '<strong>%s</strong> hits were generated.', $download_stats->total_hits, 'wp-cybertour'), number_format_i18n($download_stats->total_hits)).'</li>'."\n";
		$content .= '</ul>'."\n";
	}
	return $content;
}
...
### Function: Add wp-cybertour Top Recent Stats To WP-Stats Page
function downloadmanager_page_recent_stats($content) {
	$stats_display = get_option('stats_display');
	$stats_mostlimit = (int) get_option('stats_mostlimit');
	if( (int) $stats_display['recent_downloads'] === 1 ) {
		$content .= '<p><strong>'.sprintf(_n('%s Most Recent Download', '%s Most Recent Tours', $stats_mostlimit, 'wp-cybertour'), number_format_i18n($stats_mostlimit)).'</strong></p>'."\n";
		$content .= '<ul>'."\n";
...
### Function: Add wp-cybertour Top Most/Highest Stats To WP-Stats Page
function downloadmanager_page_most_stats($content) {
	$stats_display = get_option('stats_display');
	$stats_mostlimit = (int) get_option('stats_mostlimit');
	if( (int) $stats_display['downloaded_most'] === 1 ) {
		$content .= '<p><strong>'.sprintf(_n('%s Most Downloaded File', '%s Most Downloaded Files', $stats_mostlimit, 'wp-cybertour'), number_format_i18n($stats_mostlimit)).'</strong></p>'."\n";
		$content .= '<ul>'."\n";
...
### Class: wp-cybertour Widget
 class WP_Widget_DownloadManager extends WP_Widget {
	// Constructor
	public function __construct() {
		$widget_ops = array('description' => __('wp-cybertour downloads statistics', 'wp-cybertour'));
		parent::__construct('downloads', __('Tours', 'wp-cybertour'), $widget_ops);
	}
	...
	public function form($instance) {
		global $wpdb;
		$instance = wp_parse_args((array) $instance, array('title' => __('Tours', 'wp-cybertour'), 'type' => 'most_downloaded', 'limit' => 10, 'chars' => 200, 'cat_ids' => '0', 'link' => 1));
...
?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'wp-cybertour'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('type'); ?>"><?php _e('Statistics Type:', 'wp-cybertour'); ?>
				<select name="<?php echo $this->get_field_name('type'); ?>" id="<?php echo $this->get_field_id('type'); ?>" class="widefat">
					<option value="downloads_category"<?php selected('downloads_category', $type); ?>><?php _e('Display Tours In Category', 'wp-cybertour'); ?></option>
					<option value="recent_downloads"<?php selected('recent_downloads', $type); ?>><?php _e('Recent Tours', 'wp-cybertour'); ?></option>
					<option value="most_downloaded"<?php selected('most_downloaded', $type); ?>><?php _e('Most Downloaded', 'wp-cybertour'); ?></option>
				</select>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('limit'); ?>"><?php _e('No. Of Records To Show:', 'wp-cybertour'); ?> <input class="widefat" id="<?php echo $this->get_field_id('limit'); ?>" name="<?php echo $this->get_field_name('limit'); ?>" type="text" value="<?php echo $limit; ?>" /></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('chars'); ?>"><?php _e('Maximum Title Length (Characters):', 'wp-cybertour'); ?> <input class="widefat" id="<?php echo $this->get_field_id('chars'); ?>" name="<?php echo $this->get_field_name('chars'); ?>" type="text" value="<?php echo $chars; ?>" /></label><br />
			<small><?php _e('<strong>0</strong> to disable.', 'wp-cybertour'); ?></small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('cat_ids'); ?>"><?php _e('Category IDs:', 'wp-cybertour'); ?> <span style="color: red;">*</span> <input class="widefat" id="<?php echo $this->get_field_id('cat_ids'); ?>" name="<?php echo $this->get_field_name('cat_ids'); ?>" type="text" value="<?php echo $cat_ids; ?>" /></label><br />
			<small><?php _e('Seperate mutiple categories with commas.', 'wp-cybertour'); ?></small>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('link'); ?>"><?php _e('Display Link To Download Page?', 'wp-cybertour'); ?>
				<select name="<?php echo $this->get_field_name('link'); ?>" id="<?php echo $this->get_field_id('link'); ?>" class="widefat">
					<option value="0"<?php selected('0', $type); ?>><?php _e('No', 'wp-cybertour'); ?></option>
					<option value="1"<?php selected('1', $type); ?>><?php _e('Yes', 'wp-cybertour'); ?></option>
				</select>
			</label>
		</p>
		<p style="color: red;">
			<small><?php _e('* If you are not using any category statistics, you can ignore it.', 'wp-cybertour'); ?></small>
		<p>
...
### Function: Init wp-cybertour Widget
...
function downloadmanager_activate() {
	...
	add_option('download_template_header', '<p>'.__('There are <strong>%TOTAL_FILES_COUNT% files</strong>, weighing <strong>%TOTAL_SIZE%</strong> with <strong>%TOTAL_HITS% hits</strong> in <strong>%FILE_CATEGORY_NAME%</strong>.</p><p>Displaying <strong>%RECORD_START%</strong> to <strong>%RECORD_END%</strong> of <strong>%TOTAL_FILES_COUNT%</strong> files.', 'wp-cybertour').'</p>', 'Download Page Header Template');
	add_option('download_template_footer', '<form action="%DOWNLOAD_PAGE_URL%" method="get"><p><input type="hidden" name="dl_cat" value="%CATEGORY_ID%" /><input type="text" name="dl_search" value="%FILE_SEARCH_WORD%" />&nbsp;&nbsp;&nbsp;<input type="submit" value="'.__('Search', 'wp-cybertour').'" /></p></form>', 'Download Page Footer Template');
	add_option('download_template_category_header', '<h2 id="downloadcat-%CATEGORY_ID%"><a href="%CATEGORY_URL%" title="'.__('View all downloads in %FILE_CATEGORY_NAME%', 'wp-cybertour').'">%FILE_CATEGORY_NAME%</a></h2>', 'Download Category Header Template');
	add_option('download_template_category_footer', '', 'Download Category Footer Template');
	add_option('download_template_listing', array('<p><img src="'.plugins_url('wp-cybertour/images/ext').'/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong><a href="%FILE_DOWNLOAD_URL%">%FILE_NAME%</a></strong><br /><strong>&raquo; %FILE_SIZE% - %FILE_HITS% '.__('hits', 'wp-cybertour').' - %FILE_DATE%</strong><br />%FILE_DESCRIPTION%</p>', '<p><img src="'.plugins_url('wp-cybertour/images/ext').'/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong>%FILE_NAME%</strong><br /><strong>&raquo; %FILE_SIZE% - %FILE_HITS% '.__('hits', 'wp-cybertour').' - %FILE_DATE%</strong><br /><i>'.__('You do not have permission to download this file.', 'wp-cybertour').'</i><br />%FILE_DESCRIPTION%</p>'), 'Download Listing Template');
	add_option('download_template_embedded', array('<p><img src="'.plugins_url('wp-cybertour/images/ext').'/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong><a href="%FILE_DOWNLOAD_URL%">%FILE_NAME%</a></strong> (%FILE_SIZE%'.__(',', 'wp-cybertour').' %FILE_HITS% '.__('hits', 'wp-cybertour').')</p>', '<p><img src="'.plugins_url('wp-cybertour/images/ext').'/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong>%FILE_NAME%</strong> (%FILE_SIZE%'.__(',', 'wp-cybertour').' %FILE_HITS% '.__('hits', 'wp-cybertour').')<br /><i>'.__('You do not have permission to download this file.', 'wp-cybertour').'</i></p>'), 'Download Embedded Template');
	add_option('download_template_most', array('<li><a href="%FILE_DOWNLOAD_URL%">%FILE_NAME%</a> (%FILE_SIZE%'.__(',', 'wp-cybertour').' %FILE_HITS% '.__('hits', 'wp-cybertour').')</li>', '<li>%FILE_NAME% (%FILE_SIZE%'.__(',', 'wp-cybertour').' %FILE_HITS% '.__('hits', 'wp-cybertour').')<br /><i>'.__('You do not have permission to download this file.', 'wp-cybertour').'</i></li>'), 'Most Download Template');
	// Database Upgrade For wp-cybertour 1.30
	...
	add_option('download_template_pagingheader', '', 'Displayed Before Paging In The Tours Page');
	add_option('download_template_pagingfooter', '', 'Displayed After Paging In The Tours Page');
	add_option('download_nice_permalink', 1, 'Use Download Nice Permalink');
	add_option('download_template_download_page_link', '<p><a href="%DOWNLOAD_PAGE_URL%" title="'.__('Tours Page', 'wp-cybertour').'">'.__('Tours Page', 'wp-cybertour').'</a></p>', 'Template For Download Page Link');
	add_option('download_template_none', '<p style="text-align: center;">'.__('No Files Found.', 'wp-cybertour').'</p>', 'Template For No Tours Found');
	// Database Upgrade For wp-cybertour 1.50
